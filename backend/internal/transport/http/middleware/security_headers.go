...
func SecurityHeaders(env string, frameAncestors string) gin.HandlerFunc {
	normalizedFrameAncestors := strings.TrimSpace(frameAncestors)
	return func(c *gin.Context) {
		header := c.Writer.Header()
		setHeaderIfEmpty(header, "X-Content-Type-Options", "nosniff")
		if normalizedFrameAncestors == "" {
		...
		setHeaderIfEmpty(header, "Content-Security-Policy", contentSecurityPolicy(normalizedFrameAncestors))
...
func contentSecurityPolicy(frameAncestors string) string {
	...
	return strings.Replace(defaultContentSecurityPolicy, "frame-ancestors 'none'", "frame-ancestors "+frameAncestors, 1)
}
